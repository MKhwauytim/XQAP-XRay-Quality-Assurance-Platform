...
| `backup.complete.json` | `5-system/backups/{timestamp}/` | Written last in `createBackup`, alongside `backup.manifest.json`. Its presence marks the snapshot as fully written; nothing currently reads it back — it exists for a human to notice on disk. |
| `restore.inprogress.json` | `5-system/` | Written before a restore's file-copy walk starts, removed only once that walk finishes successfully. A restore interrupted partway (crash, tab close, thrown error) leaves this file behind, making the interruption detectable on disk; nothing currently reads it back automatically. |
