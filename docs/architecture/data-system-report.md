...
| `backup.manifest.json` and copied JSON data files | `5-system/backups/{timestamp}/` | Manual/automatic restorable snapshots. Every data file is copied as BYTES and then re-read and verified against the bytes that were copied (`safeWrite.copyFileBytesVerified`) — never decoded to text, so an oversized or compressed file can no longer fail the whole walk. A copy that cannot be verified does not abort the snapshot: the manifest's optional `status` becomes `partial` and names the file in `filesFailedVerification`, and a file that vanished mid-walk is listed in `filesSkippedMissing`. Manifests written before those fields existed read as `complete`. Routine backups do not rebuild redundant XLSX exports; spreadsheet generation is an explicit compatibility option. |
| `backup.complete.json` | `5-system/backups/{timestamp}/` | Written last in `createBackup`, alongside `backup.manifest.json`. Its presence marks the snapshot as fully written, and `assertBackupComplete` refuses to restore a folder that lacks it. |
| `restore.inprogress.json` | `5-system/` | Written before a restore's file-copy walk starts, removed only once that walk finishes successfully. A restore interrupted partway (crash, tab close, thrown error) leaves this file behind. `restoreSentinel.ts` reads it back and `RestoreWarningBanner` warns every user of the mounted workspace once it is older than `RESTORE_SENTINEL_STALE_AFTER_MS` (15 min) — the delay is what keeps a restore genuinely still running on another machine from being reported as interrupted. |
...
| `{stem}.errors.json` | `5-system/system-errors/` | Persistent, admin-exportable error log (`src/data/errorLog/`, added v116/v117 — see the plan at `docs/superpowers/plans/2026-08-24-error-log-subsystem-plan.md`). ONE file per user (`{stem}` = `auditUserStem(username)`, the same collision-resistant hash-suffixed stem `audit/` uses), never a shared file — two machines can never contend on the same entry. Written via `casLoop` (6 retries, 100 ms base delay — copied from the audit trail's own SMB tuning) under a per-user `withResourceLock`. Live entries are capped at 2,000 per user; overflow is archived to that user's per-year archive file BEFORE the live log is trimmed, and archive failure blocks the trim so no entry is ever dropped unarchived. The persisted `UserErrorLogFile`/`PersistedErrorEntry` shapes mirror the in-memory `ErrorEntry` (`errorLogger.ts`) plus an `id` and the acting `username`/`role`. Fed by a batching in-memory sink (`errorLogSink.ts`) registered into `errorLogger.ts` via `registerErrorSink` — `errorLogger.ts` itself never imports the workspace layer, to avoid a module cycle through `safeWrite.ts`. Deliberately carries NO shared legacy file (this subsystem has never had one) and NO `previousArchiveHash` hash chain — see the next row and the `docs/architecture/SECURITY_MODEL.md` note. |
| `{stem}.errors.{year}.json` | `5-system/system-errors/` | Per-user per-year archive of error entries evicted from that user's live `{stem}.errors.json` once it exceeds its 2,000-entry cap. Idempotent by entry `id`, so a `casLoop` retry cannot double-append. Unlike the audit trail's `actions.archive.{year}.json`, this file carries **no `previousArchiveHash` chain** — deliberately: the audit trail's B5 chain is a tamper-evident property that matters for governance evidence about people, and this log is diagnostic telemetry about software, so adding the chain would imply a guarantee this data does not have. **No migration, no legacy predecessor, and rollback is "stop writing" — the folder is read-only-forever thereafter**, the same doctrine `auditPaths.ts` documents for the legacy shared audit files, applied in reverse (there was never a shared file to begin with). |
