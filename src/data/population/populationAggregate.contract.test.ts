 ...
 * `row.<field>` access and fails if one isn't listed in the aggregate's
 * preview-row field set -- the same failure mode the owner's requirement
 * warns about: a missing field silently forces a fallback to reading rows.
 */
describe("POPULATION_AGGREGATE_PREVIEW_FIELDS contract", () => {
  it("covers every `row.<field>` access in PopulationProcessingReport's preview table", () => {
