...
/* A 16:9 landscape slide, 1120x630px at the design width. Height is a FIXED
   ...
   for real narrow/mobile viewports; the max-width:1152px query below
   ...
   That query's threshold used to be 900px, which left a dead zone: between
   900px and 1120px the card was narrower than its 1120px design width but
   still pinned to 630px, so the row-budget math overflowed and this rule's
   overflow:hidden silently cut it. Browser zoom moves the effective CSS
   viewport width exactly like a resize, so ordinary zoom levels landed in
   that band. Raising the threshold to the design width closes it: at or
   above 1120px you get the framed card, below it the auto-height flow. */
.slide{
...
@media print{
  ...
  /* Blanket rule, not just body{} — print-color-adjust is nominally an
     inherited property, but this codebase's own history (see the narrow
     per-element overrides added in deck2/theme.ts: .v2-bar-cell,
     .v2-prop-seg, .v2-num-tile, etc.) shows that relying on inheritance
     alone left real elements printing without their background colors.
     A blanket star-selector rule (same fix applied to deck3/theme.ts)
     can't miss one. */
