...
import { buildAssignEvent, buildReassignEvent, computeDaysRemainingForDeadline } from "./distributionLog";
...
export type BulkReassignExecuteResult = {
  ok: boolean;
  /** Ids a reassignment event was durably written for on THIS call. */
  appliedIds: string[];
  /** Ids the batch had already durably reassigned on an earlier (partial) attempt with the same sourceRequestId — safe/expected on retry. */
...
/**
 * Reassign a batch of already-distributed rows to `reassignedTo`, going
 * through the same append-only distribution event log every other mutation in
 * this app uses (buildReassignEvent + appendDistributionEvents) — never
 * writes derived state directly.
 *
 * Safety:
 *  - Always re-derives distribution state from a FRESH read
 *    (`loadOrDeriveDistributionCurrent`, not the `...ForRead` cached variant)
 *    immediately before planning — never trusts a caller's possibly-stale
 *    rendered snapshot. This is exactly where a stale read would double-assign
 *    at scale.
 *  - `sourceRequestId` is a caller-supplied idempotency key (stable across
 *    retries of the same user action, e.g. re-clicking "confirm" after a
 *    partial failure). Before planning, the current event log is scanned for
 *    "reassigned" events already carrying this key and targeting the same
 *    `reassignedTo`; those ids are excluded from re-emission and reported back
 *    as `alreadyAppliedIds` — mirrors the replay guard in
 *    `referral/approveReferral.ts`'s `approveReferral`.
 *  - Month-lock (`ensureMonthWritable`) is enforced inside
 *    `appendDistributionEvents` itself; a closed month rejects with
 *    `MonthClosedError`, which this function does not catch — callers handle
 *    it the same way every other mutating handler in this app does.
 */
export async function executeBulkReassignment(params: {
...
}): Promise<BulkReassignExecuteResult> {
  const { directoryHandle, monthFolderName, xrayImageIds, reassignedTo, eventBy, reason, sourceRequestId } = params;

  if (xrayImageIds.length === 0) {
    return { ok: true, appliedIds: [], alreadyAppliedIds: [], skipped: [] };
  }
  ...
  // Replay guard (idempotency for a retried/partial bulk action) — see docblock above.
  const distLog = await loadDistributionLog(directoryHandle, monthFolderName);
  const alreadyAppliedIds = new Set(
    distLog.events
      .filter(
        (event) =>
          event.sourceRequestId === sourceRequestId &&
          event.eventType === "reassigned" &&
          event.reassignedTo === reassignedTo
      )
      .map((event) => event.xrayImageId)
  );

  const pendingIds = xrayImageIds.filter((id) => !alreadyAppliedIds.has(id));
  const plan = planBulkReassignment(current.entries, pendingIds, reassignedTo);

  if (plan.eligible.length === 0) {
    return { ok: true, appliedIds: [], alreadyAppliedIds: [...alreadyAppliedIds], skipped: plan.skipped };
  }

  const events = plan.eligible.map((row) =>
    buildReassignEvent({
  ...
  return {
    ok: true,
    appliedIds: plan.eligible.map((row) => row.xrayImageId),
    alreadyAppliedIds: [...alreadyAppliedIds],
    skipped: plan.skipped,
  };
}
