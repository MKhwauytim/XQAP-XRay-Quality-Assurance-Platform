...
export function ActivitySection(props: {
  ...
  const latestEntries = props.entries.slice().sort((a, b) => Date.parse(b.lastSeenAt) - Date.parse(a.lastSeenAt)).slice(0, 100);
...
export function ActionsSection(props: {
  ...
  const entries = props.entries.slice().sort((a, b) => Date.parse(b.at) - Date.parse(a.at)).slice(0, 200);
  return <div className="um-section">
