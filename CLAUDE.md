...
- `vite-plugin-singlefile` inlines everything (`assetsInlineLimit` maxed, `cssCodeSplit: false`): the build produces one portable `dist/index.html` — ~3.87 MB raw / ~1.25 MB gzip as of v123.1. **The ceiling is 30 MB raw / 10 MB gzip**, set by the owner on 2026-08-25 (`scripts/check-bundle-size.mjs`, which carries the full reasoning). That is a hard stop with a lot of room, not a target: the app is copied onto a Windows share and opened as `file://`, so there is no content-encoding negotiation and every raw byte is read off the share on every open. The regression guard is no longer the ceiling but the `Measure (pr)` CI job, which reports raw/gzip Δ against `main` on every push — read that delta rather than assuming headroom means free. `npm run check:bundle-size` remains the release gate.
...
**Post-launch migration policy — binding on future workspace-format changes.** The app has shipped and real workspaces are now in active use; the "no active schema migration" statement above describes today's code, not a standing exemption for new work. From here on, any change that alters a workspace file's on-disk shape (a `JsonEnvelope.schemaVersion` bump, a new required field, a renamed/relocated folder, a changed file layout) must ship its own migration rather than another permanent legacy-fallback read path: when an admin signs into an existing workspace still carrying the old shape, the app offers to migrate it (never triggered silently, and never for a non-admin role); running it migrates every affected file to the new shape, and the pre-migration originals are ARCHIVED — never hard-deleted — following the precedent `feedbackStorage.ts`'s `finalizeLegacyMigration` already sets (`messages.json` → `messages.json.migrated`). Write each migration as its own explicit, tested module, not an inline branch buried in a read function.

Both previously documented drifts have since been **fixed in code**. **`4-reports/` is not empty** — Report Designer (`src/data/reportDesigner/storage/reportDesignStorage.ts`) writes `designs.index.json` (CAS-protected) and per-design `{reportId}.json` under `4-reports/designs/` via `getReportsRoot(..., true)`. **`feedback/` now writes under `5-system/feedback/`, one file per conversation** — `threads/{threadId}.json` is the source of truth, `threads.index.json` a rebuildable CAS-protected summary cache written on create/status-change and, since v122.1, by an **opt-in, rate-limited** index repair that only a user-initiated feedback surface requests (`listThreadSummaries(dir, { repairIndex: true })`) — the repair used to run from the plain read path, which made every client's unread poll a writer of the one shared file on every page in the app; and the pre-v116 shared `messages.json` is read-only and migrated out of lazily on first read. `feedbackStorage.ts` still *reads* the legacy top-level `feedback/` root so pre-existing workspaces keep working, and a real admin can additionally run `finalizeLegacyMigration` (FeedbackWidget's admin "all messages" tab) to verify the copy-out is complete and then ARCHIVE — never hard-delete — `messages.json` to `messages.json.migrated`, at whichever location(s) hold a live copy. The background unread poll in `FeedbackUnreadProvider` (`loadFeedback`, which opens every thread file) was widened from 60 s to a 5-minute backstop and made visibility-gated (skipped while the tab is hidden, caught up on `visibilitychange`) as of 2026-08-25, since it grows with total ticket count and the 45 s bounded-signature tick in `workspaceSync` already covers the common "reply to a recent thread" case with low latency. See `docs/architecture/data-system-report.md` for the current picture.
...
- **Every matrix, KPI, or measure shown in a report must already exist in the app and be mapped from the app's own data** — reports (executive deck, KPI tab, Report Designer fields, exports) may only surface figures traceable to real fields in the population/sample/distribution/answers data layer. Never invent, hardcode, or fabricate a metric to make a report look complete; if the underlying data doesn't exist yet, add it to the data layer first (or leave the metric out) rather than faking the report.
- **No duplicate state — one place, one name, called everywhere.** A shared value (a logo, a color, a constant, a config flag, a piece of derived data) is defined in exactly one module and imported by every consumer. Never re-declare the same thing under a different name in multiple files/components "just for this page" — that's how the branding logo, a threshold, or a label ends up with three slightly-different copies that drift. Before adding a new constant/variable, grep for an existing one that already means the same thing and reuse or extend it instead.
- **Close out the session after the PR lands.** Once a PR opened for this repo has merged into main and its branch is closed/deleted, archive the Claude Code session that did the work — don't leave a finished session sitting active.
